...
sw_xor = sm.LogicGate(blueprint,ID,"xor",(1,3,1),"up","up") # sw_xor as output and flip flop
tick_and = sm.LogicGate(blueprint,ID,"and",(1,2,1),"up","up") # and gate for 1 tick pulse
tick_nor = sm.LogicGate(blueprint,ID,"nor",(1,1,1),"up","up") # nor gate for 1 tick pulse
timer = sm.Timer(blueprint,ID,0,39,(2,2,1),"north","up") # timer to set the pulse length
button = sm.Button(blueprint,ID,(2,1,1),"up","up") # button to trigger the pulse
